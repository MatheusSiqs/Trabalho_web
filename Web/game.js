...
function criaTabuleiro(cartas){
	...
	for(let i = 0; i<tamanho;i++){
		...
		for(let k = 0;k<tamanho;k++){
			...
			costa.setAttribute("src","Pares/Par"+cartas[cons+k]+".jpg");
			
			frente.alt="Carta"+cartas[cons+k];
			frente.id="C"+num;
...
function verificaParidade(event){
  ...
  if(!jaEscolheu && modo == 'temp'){
	...
	if((carta.alt != null) && (carta.alt != "costa")){
		flipCarta(event.target.id)
		if(temp == null){
			temp = [carta.alt,carta.id]
		}else{
			if(temp[0] == carta.alt){
				temp = null;
...
function finalizaJogo(){
  ...
  request.open("post","cadastraResultados.php",true);
  ...
  request.onload = function (e) {
    if (request.readyState === 4) {
      if (request.status === 201) {
        ...
        alert("não foi possível salvar seu resultado");
      }
  };
}
  
  request.send(JSON.stringify({
    idUser: jogador.innerHTML,
    tempo: timer.innerHTML,
    pontuacao: pontos.innerHTML,
    modo: modo,
    tamanho: tamanho,
    status: 'D',
  }));
  ...
  if(confirm("Você perdeu.\nDeseja jogar novamente?")){
    ...
    window.open("../Ejogo/Ejogo.php","_self");
  }
}
